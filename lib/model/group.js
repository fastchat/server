...
Group.statics = {
  ...
  validateMembers: function(members, user) {
    ...
    return User.find({'username': { $in: members } })
      .execQ().then(function(users) {
  ...
  newGroup: function(members, user, message, name) {
    ...
    return self.validateMembers(members, user).then(function(members) {
      return [members, self.newGroup({name: name, members: members})]
    }).spread(function(members, group) {
	...
	if (!io.emitNewGroup(user._id, group)) {
