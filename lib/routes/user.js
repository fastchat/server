...
exports.postAvatar = function(req, res, next) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    if (!files) return next('No files were found in the upload!');
    if (!files.avatar) return next('Avatar was not found in the upload!');

    var file = files.avatar[0];
    if (!file) return next('Avatar was not found in the files, in first index!');
  ...
  User.findOne( { '_id' : userId }, function(err, user) {
    ...
    knox.get(user.avatar).on('response', function(s3res){
      ...
      s3res.on('end', function() {
        res.contentType('image/jpeg'); //this is lying
